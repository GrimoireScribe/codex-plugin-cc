...
test("continue is not exposed as a user-facing command", () => {
  const commandFiles = fs
    .readdirSync(path.join(PLUGIN_ROOT, "commands"), { withFileTypes: true })
  ...
  assert.deepEqual(commandFiles, [
    ...
    "result.md",
    "review.md",
    "scoping-adversarial-review.md",
    "setup.md",
    "spec-adversarial-review.md",
    "status.md",
    "transfer.md"
  ]);
});
...
test("rescue command absorbs continue semantics", () => {
  ...
  assert.match(rescue, /--model <model\|astra\|sol\|luna>/);
  ...
  assert.match(rescue, /the plugin default \(`gpt-6-astra` at `medium`\) applies/i);
  assert.match(rescue, /`astra`, `sol`, and `luna` map to `gpt-6-astra`, `gpt-6-sol`, and `gpt-6-luna`/i);
  ...
  assert.match(agent, /`astra`, `sol`, and `luna` map to `gpt-6-astra`, `gpt-6-sol`, and `gpt-6-luna`/i);
  assert.match(agent, /If the user asks for a concrete model name such as `gpt-6-sol`, pass it through with `--model`/i);
  assert.match(agent, /Return the stdout of the `codex-companion` command exactly as-is/i);
  assert.match(agent, /If the Bash call fails.*retry ONCE/i);
  assert.match(agent, /gpt-prompting/);
  ...
  assert.match(runtimeSkill, /use the `gpt-prompting` skill to rewrite the user's request into a tighter Codex prompt/i);
  assert.match(runtimeSkill, /That prompt drafting is the only Claude-side work allowed/i);
  assert.match(runtimeSkill, /Leave `--effort` unset unless the user explicitly requests a specific effort/i);
  assert.match(runtimeSkill, /Leave model unset by default/i);
  assert.match(runtimeSkill, /the companion normalizes `astra`\/`sol`\/`luna` to `gpt-6-astra`\/`gpt-6-sol`\/`gpt-6-luna` itself/i);
  ...
  assert.match(readme, /if you do not pass `--model` or `--effort`, the plugin uses its own default \(`gpt-6-astra` at `medium`\)/i);
  assert.match(readme, /--model luna --effort medium/i);
  assert.match(readme, /`astra`, `sol`, and `luna` are shorthands the plugin maps to `gpt-6-astra`, `gpt-6-sol`, and `gpt-6-luna`/i);
...
test("transfer, result, and cancel commands are exposed as deterministic runtime entrypoints", () => {
  ...
  assert.match(result, /codex-companion\.mjs" result/);
  assert.match(cancel, /disable-model-invocation:\s*true/);
  assert.match(cancel, /codex-companion\.mjs" cancel/);
...
test("internal docs use task terminology for rescue runs", () => {
  const runtimeSkill = read("skills/codex-cli-runtime/SKILL.md");
  const promptingSkill = read("skills/gpt-prompting/SKILL.md");
  const promptRecipes = read("skills/gpt-prompting/references/codex-prompt-recipes.md");
