...
- Refresh the model registry for the GPT-6 lineup (`gpt-6-astra`, `gpt-6-sol`, `gpt-6-luna`) and remove the dead `gpt-5.3-codex-spark`, `gpt-5.4-mini`, and `gpt-5.4` models everywhere: alias map, fast-tier set, help text, command argument-hints, rescue command/agent/skill instructions, `gpt-5-4-prompting` skill (renamed to `gpt-prompting`, `git mv`'d with history intact), README, and tests. Aliases are now `astra`/`sol`/`luna` only (case-insensitive) — no `terra` alias, so `--model terra` still passes through literally, same as any unrecognized model name. Fast-tier (small-model safety) membership is now exactly `{gpt-6-luna, gpt-5.6-terra, gpt-5.6-luna}`; `gpt-6-astra`, `gpt-6-sol`, `gpt-5.6-sol`, and `gpt-5.5` are not fast tier.
- Add a plugin-level default model and reasoning effort — `gpt-6-astra` at `medium` — applied by every command that launches a Codex turn (`task` foreground/background/resume and the stop-gate review task, `review`, `adversarial-review`, `spec-adversarial-review`, `scoping-adversarial-review`). Precedence is explicit `--model`/`--effort` flag > `CODEX_DEFAULT_MODEL`/`CODEX_DEFAULT_EFFORT` env var > built-in default; `~/.codex/config.toml`'s own `model`/`model_reasoning_effort` are no longer consulted on any path this plugin launches, since the plugin always passes `--model` and `model_reasoning_effort` explicitly now. `CODEX_DEFAULT_MODEL` goes through the same alias normalization as `--model`; `CODEX_DEFAULT_EFFORT` goes through the same validation as `--effort` and throws a clear error naming the env var on an invalid value. The defaults are resolved once in the command handlers (`handleTask`, `handleReviewCommand`, `handleSpecAdversarialReview`, `handleScopingAdversarialReview`) and stored in the job request, so a background worker (`handleTaskWorker`) re-runs with the already-resolved model/effort instead of re-resolving. `review`/`adversarial-review` gained a `--effort` flag (previously accepted none), threaded through `executeReviewRun` into `runCodexExecTask`. For fast-tier spec/scoping reviews, the effort cap now sees the effort the caller supplied (flag, else env) BEFORE the built-in default is filled in, so a fast-tier model with no flag and no env still lands on the capped `high` instead of silently dropping to the built-in `medium` — matching current PO fast-tier wrapper behavior. `--effort none` is a valid explicit choice and is passed through, not treated as unset.
