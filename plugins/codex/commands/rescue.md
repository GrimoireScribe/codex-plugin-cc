...
argument-hint: "[--background|--wait] [--resume|--fresh] [--model <model|astra|sol|luna>] [--effort <none|minimal|low|medium|high|xhigh>] [what Codex should investigate, solve, or continue]"
...
- Leave the model unset unless the user explicitly asks for one. When unset, the plugin default (`gpt-6-astra` at `medium`) applies. The shorthands `astra`, `sol`, and `luna` map to `gpt-6-astra`, `gpt-6-sol`, and `gpt-6-luna` (the companion also normalizes them on its own).
